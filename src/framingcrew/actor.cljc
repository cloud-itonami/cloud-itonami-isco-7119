(ns framingcrew.actor
  "FramingCrewActor — the ISCO-08 7119 building frame and related
  trades workers job-site scheduling/logistics coordination actor as
  a `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one scheduling/logistics operation
  ...
  The unconditional invariant: the FramingCrewAdvisor can never
  directly commit a record the FramingCrewGovernor refuses — every
  commit-record! call is gated behind `:decide`, and the governor's
  closed op-allowlist and scope-exclusion check ensure that no
  proposal can finalize a structural-framing-execution decision or
  override a site-safety officer's judgment."
...
(defn build-graph
  "Build a compiled FramingCrewActor graph. `store` implements
  `framingcrew.store/Store`. `advisor` implements
  `framingcrew.advisor/Advisor` (defaults to `mock-advisor`).
